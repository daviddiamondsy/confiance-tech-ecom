...
export async function PUT(req: NextRequest) {
  ...
  if(cheapYuanThreshold != null && cheapSellingMarkup == null) {
    return NextResponse.json({ error: "Cheap markup multiplier is required when yuan threshold is set" }, { status: 400 });
  }

  if (cheapWholesaleNgnThreshold != null && cheapSellingMarkup == null) {
    return NextResponse.json({ error: "Cheap markup multiplier is required when wholesale threshold is set" }, { status: 400 });
  }
